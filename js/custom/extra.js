//navbar scroll yapıldığında sabitleme
window.addEventListener("DOMContentLoaded",event=>{
    var navbarMobile=function(){
        ...
        if(!nCollapsible){
            return;
        }else if(window.scrollY< myHeaderHeight){
...
//mobil navbara tıkladığında geri kapatma
var NavBar=document.querySelectorAll(".btn-close-canvas");
...
(function (){
    ...
    function controlName(){
        ...
        if(myName.value.length==0){
            ...
            myError.textContent="Bu alan boş bırakılamaz";
            return false;           
    ...
    function controlEmail(){
        ...
        if (myEmail.value.length == 0) {
            ...
            return false;
        } else if (myEmail.value.length > 90) {
            myName.classList.remove("is-valid");
            myName.classList.add("is-invalid");
            myError.textContent = "E-postanız 30 karakterden fazla olamaz";
            return false;
    ...
    myForms.addEventListener("submit",function(e){
        ...
        !controlPhone() ||
        !controlMessage()){
            e.preventDefault();//olayları gönderme
            e.stopPropagation();// hepsini durdur
        }
    },false)
})();